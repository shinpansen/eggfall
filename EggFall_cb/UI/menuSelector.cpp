...
void menuSelector::PerformInputs(RenderWindow* window)
{
    ...
    //Keyboard
    if((_input->isKeyPressed(CMD_LEFT) || _input->isKeyPressed(CMD_RIGHT)) && !_keyPressed)
    {
        _selectedItem += _input->isKeyPressed(CMD_LEFT) ? -1 : 1;
        itemHasChanged = true;
        _keyPressed = true;
    ...
    else if(!_input->isKeyPressed(CMD_LEFT) && !_input->isKeyPressed(CMD_RIGHT) && !Mouse::isButtonPressed(Mouse::Left))
        _keyPressed = false;
...
void menuSelector::PerformSelectedItem(float deltaTime)
{
    //Select item
    for(int i=0 ; i<_items.size() ; i++)
    {
		...
		if (_items[i]->getScale().x > 1)
			_items[i]->setScale(_items[i]->getScale().y - 0.1*deltaTime, _items[i]->getScale().y - 0.1*deltaTime);
		else
			_items[i]->setScale(1, 1);
    }
    ...
    _leftArrow.setScale(1.8+scaleLeft/50.f, 0.8+scaleLeft/50.f);
    _leftArrow.setFillColor(Color(255,255,255,135+scaleLeft*8));
    _rightArrow.setScale(1.8+scaleRight/50.f, 0.8+scaleRight/50.f);
...
void menuSelector::Display(RenderWindow* window, float deltaTime, Sprite* backItemSprite, Sprite* backItemSprite2)
{
    ...
    if(_backShape.getScale().x < 1)
        _backShape.setScale(_backShape.getScale().x+0.2f*deltaTime, 1);
    else
    {
		_backShape.setScale(1, 1);
        PerformSelectedItem(deltaTime);
        if(!_lockSelection)
        {
