...
fadeOption::fadeOption(Vector2f directionStep, float opacityStep, float rotationStep, float durationStep, float speedReduction, Vector2f scaleStep, float opacityDelay)
{
	// Full fade option
				     ...
				     float rotation, Vector2f pos, Vector2f origin, float opacity, fadeOption fadeOption)
{
	...
	_animation->SetAnimation(numAnimation, nbFrames);
...
void effectObj::Step(float deltaTime)
{
	try
	{
		...
		if (outsideScreen || endAnimation || _opacity <= 0.f || _fadeOption.duration == 0.f)
			_finished = true;
	}
	catch (exception e)
	{
