...
public:
	fadeOption();
	fadeOption(Vector2f directionStep, float opacityStep, float rotationStep, float durationStep, float speedReduction = 0, Vector2f scaleStep = Vector2f(0,0), float opacityDelay = 0.f);
...
public:
	...
	effectObj(Texture* texture, Vector2f tileSize, float frameDelay, int nbFrames, int numAnimation, bool reversed, float rotation, Vector2f pos, Vector2f origin, float opacity, fadeOption fadeOption);
