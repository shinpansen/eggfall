	...
	try
	{
		...
		for (b2Body* body = _world->GetBodyList(); body != 0; body = body->GetNext())
		{
				...
				body->GetPosition().y*SCALE < position.y)
			{
				...
				_angleToReach = (body->GetPosition().x*SCALE - position.x)*(2.f * yDistance);
				_angleToReach = fabs(_angleToReach) > 60.f ? _angleToReach / fabs(_angleToReach)*60.f : _angleToReach;
				break;
			}
		}
