...
void StepGame(mainGame* theGame, bool * physicsThreadBusy)
{
    //Initialization
    Clock clock;
    float deltaTime = 1.0;
...
int main(int argc, char **argv)
{
    ...
    Thread * physicsThread = new Thread(bind(&StepGame, &theGame, &physicsThreadBusy));
    ...
    window.setView(view);
	//window.setFramerateLimit(30); //Bad idea
    ...
    while (window.isOpen())
    {
        ...
        if(!gamePaused)
        {
            ...
            //Display the game
			theGame.Display(key, keyPressed, displayDeltaTime);
