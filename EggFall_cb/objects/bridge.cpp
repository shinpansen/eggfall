...
bridge::bridge(b2World * world, Texture * texture, int colorID, Vector2f posLeft, Vector2f posRight)
{
	...
	_leftBody = Box2DTools::CreateRectangleBox(_world, Vector2f(_posLeft.x - 8, _posLeft.y + 8), 16, 16, 0, 0, 0, Box2DTools::GROUND_ENTITY);
	...
	_rightBody = Box2DTools::CreateRectangleBox(_world, Vector2f(_posRight.x + 8, _posRight.y + 8), 16, 16, 0, 0, 0, Box2DTools::GROUND_ENTITY);
	...
	for (int i = 0; i < nbBlocks; i++)
	{
		...
		//Box2D bodies
		Vector2f blockPos = Vector2f(_posLeft.x + 8 + i * 16, _posLeft.y - 1);
