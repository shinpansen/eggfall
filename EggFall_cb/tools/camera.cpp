...
{
    int minX = heros[0]->GetPositionA().x;
    int minY = heros[0]->GetPositionA().y;
    for(int i=1 ; i<heros.size() ; i++)
    {
        if(heros[i]->GetPositionA().x < minX)
            minX = heros[i]->GetPositionA().x;
        if(heros[i]->GetPositionA().y < minY)
            minY = heros[i]->GetPositionA().y;
    }
...
{
    int maxX = heros[0]->GetPositionA().x;
    int maxY = heros[0]->GetPositionA().y;
    for(int i=1 ; i<heros.size() ; i++)
    {
        if(heros[i]->GetPositionA().x > maxX)
            maxX = heros[i]->GetPositionA().x;
        if(heros[i]->GetPositionA().y > maxY)
            maxY = heros[i]->GetPositionA().y;
    }
