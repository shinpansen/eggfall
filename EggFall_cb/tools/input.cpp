...
input::input(int player, int deadZone, RenderWindow* window, bool analogControls)
{
    _deadZone = deadZone;
	_runTimeoutA = 0.f;
	_runTimeoutB = 0.f;
...
bool input::isKeyPressed(COMMAND cmd)
{
    if(_player >= 0)
    {
        switch(cmd)
        {
			...
			case CMD_AIM:
				return Keyboard::isKeyPressed(_KEY_AIM) || Joystick::isButtonPressed(_player, _JOY_AIM);
            case CMD_UP:
...
{
	if (!isKeyPressed(CMD_ANY_DIRECTION))
		return Vector2f(0, 0);
...
void input::Step(float deltaTime)
{
	//Double press on direction to run
	if (!_runKeyPressed && (isKeyPressed(CMD_LEFT) || isKeyPressed(CMD_RIGHT)))
	{
		_runKeyPressed = true;
		_runTimeoutA = 12.f; //Force to always double tap direction to run, instead of single pressing after a long press
