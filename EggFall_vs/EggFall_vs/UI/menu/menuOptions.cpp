...
void menuOptions::UpdateOptions(vector<string> options, bool isKeys)
{
    ...
    for(int i=0 ; i<_options.size() ; i++)
    {
		...
		string option = "";
		if (_options[i] == "Francais")
			option = "Français";
		else if (_options[i] == "Espanol")
			option = "Espaµol";
		else
			option = _options[i];

        _texts[id]->setFont(_font);
        _texts[id]->setString(isKeys ? _input->GetKeyName(key) : option);
        _texts[id]->setCharacterSize(36);
        _texts[id]->setFillColor(Color(207,0,205));
    ...
    _shape.setPosition(_pos.x, _shape.getPosition().y);
