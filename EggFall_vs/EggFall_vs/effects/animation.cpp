...
animation::animation(Sprite* tile, float frameDelay, int width, int height, int nbFrames, int nbAnimations, 
					 int staticAnimation, bool disableScale, int nbPages)
{
    //Variables
    _tile = tile;
    _width = width;
	...
	_nbPages = nbPages;
	_nbAnimationsPerPage = nbAnimations / (nbPages != 0 ? nbPages : 1);// tile->getTexture()->getSize().y / height;
    _animation = -1;
    _loop = 0;
    _frame = 0;
    _deltaTime = 1;
...
{
	int numTile = (int)((float)(numAnimation) / (float)_nbAnimationsPerPage);
	return numTile < _nbPages ? numTile : 0;
}
...
{
    return ((int)(_frame + _frameDelay*_deltaTime) >= _nbFrames);
}
...
void animation::PerformAnimation(float deltaTime)
{
    try
    {
		...
		if ((int)(_frame + _frameDelay*_deltaTime) >= _nbFrames)
			_frame = (float)_loop;
		else
		...
		int leftRect = 0;
		if (_nbAnimations > _nbAnimationsPerPage && _nbPages > 1)
		{
			int numTile = GetNumTile(_animation);
			animationTile = _animation - (_nbAnimationsPerPage*numTile);
			leftRect = _width * _nbFrames * numTile;
		}
        _borderTopLeft = Vector2i((int)_frame*_width + leftRect, animationTile*_height);
