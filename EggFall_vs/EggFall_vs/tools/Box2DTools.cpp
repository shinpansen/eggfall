...
b2Body * Box2DTools::CreateBody(b2World* world, Vector2f pos, Vector2f shapeSize, float friction, float density, float angle, short category, SHAPE shape, bool bridge)
{
	...
	if (category == PARTICLE_ENTITY)
		...
		fixtureDef.filter.maskBits = BODY_ENTITY | PARTICLE_ENTITY;
	else
...
bool Box2DTools::TestLadderPoint(b2World * world, b2Vec2 point)
{
	//Testing world
	if (world == NULL) { return false; }
	...
	{
		//Can only hook to static bodies
		if (BodyIterator->GetFixtureList()->GetFilterData().categoryBits != LADDER_ENTITY)
			continue;
		else if (BodyIterator->GetFixtureList()->TestPoint(point))
			return true;
	}
	return false;
}
