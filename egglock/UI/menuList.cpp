...
menuList::menuList(vector<string> items, Vector2f pos, input* input, bool showBackground, string alignment)
{
    //Objects - variables
    _deltaTime = 1;
    _pos = pos;
    ...
    _topLeftTexture.loadFromFile(_files.menuTopLeft);
    _topRightTexture.loadFromFile(_files.menuTopRight);
    _bottomLeftTexture.loadFromFile(_files.menuBottomLeft);
    _bottomRightTexture.loadFromFile(_files.menuBottomRight);
    _middleBottomTexture.loadFromFile(_files.menuMiddleBottom);
    _duplicateTexture.loadFromFile(_files.menuDuplicate);
...
void menuList::ResetAnimation(bool resetItemPos)
{
    ...
    _shape.setScale(0,0);
    _background.setSize(Vector2f(800,600));
    _background.setOrigin(400,300);
    _background.setPosition(400,300);
...
void menuList::PerformSelectedItem(bool forceFocus)
{
    for(int i=0 ; i<_texts.size() ; i++)
    {
        ...
        _texts[i]->setColor(i == _selectedItem ? Color(255,205,0) : Color(255,255,255));
...
void menuList::Display(RenderWindow* window, float deltaTime, bool step, bool borders)
{
    ...
    window->draw(_shape);
    if(_shape.getScale().x <= 0.9) //Perform menu animation
    {
